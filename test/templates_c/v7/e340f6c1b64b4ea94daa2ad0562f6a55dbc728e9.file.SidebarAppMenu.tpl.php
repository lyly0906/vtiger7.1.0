<?php /* Smarty version Smarty-3.1.7, created on 2018-06-07 05:52:40
         compiled from "D:\php\vtigercrm7.1.0\includes\runtime/../../layouts/v7\modules\Vtiger\partials\SidebarAppMenu.tpl" */ ?>
<?php /*%%SmartyHeaderCode:209125b17b0ff36a383-27878533%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    'e340f6c1b64b4ea94daa2ad0562f6a55dbc728e9' => 
    array (
      0 => 'D:\\php\\vtigercrm7.1.0\\includes\\runtime/../../layouts/v7\\modules\\Vtiger\\partials\\SidebarAppMenu.tpl',
      1 => 1528350756,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '209125b17b0ff36a383-27878533',
  'function' => 
  array (
  ),
  'version' => 'Smarty-3.1.7',
  'unifunc' => 'content_5b17b0ff692d0',
  'variables' => 
  array (
...
<?php if ($_valid && !is_callable('content_5b17b0ff692d0')) {function content_5b17b0ff692d0($_smarty_tpl) {?>
...
<script>
	$(function() {
		$('input[name=department]').click(function () {
            var rolename = $('#select2-chosen-2').text();
			var obj = {};
			obj.rolename = rolename;
			$.ajax({
				type: "GET",
				url: "index.php?module=Departments&parent=Settings&action=BasicAjax",
				data : obj,
				dataType: "json",
				success: function (data) {
					//console.log(data);
					var html = '';
					html = "<select id='depa' readOnly='true' class='inputElement' >";
					...
					$.each(data.result, function (index, content) {
						...
						if (content.isshow == 0){
							...
							content.name += content.name + ' <i style="color:red;font-size:10px;">聊天平台还未创建该部门</i>';
						}

						for(i=1;i<content.depth * 1;i++){
							if(content.company > 0)
								blank += '-';
						}

						if(content.depth == 2 && content.company > 0) {
							html += "<option value='" + content.name + "' style='font-weight:bold;' " + isshow + ">" + blank + content.name + "</option>";
							//console.log(content['name']);
						}else {
							html += "<option value='" + content.name + "' " + isshow + ">" + blank + content.name + "</option>";
						}
					});
					html += "</select>";
					$('#depa').remove();
