...
if($activevalue[0] == 1)
{
	...
	while ($myrow = $adb->fetch_array($result))
	{
		...
		$mail_body = getTranslatedString('Dear_Admin_tasks_not_been_completed')." ".getTranslatedString('LBL_SUBJECT').": ".$subject."<br> ".getTranslatedString('LBL_ASSIGNED_TO').": ".$assigned_user."<br><br>".getTranslatedString('Task_sign');
	 	sendmail($emailaddress,$mailuname,getTranslatedString('Task_Not_completed').': '.$subject,$mail_body,$mailserver,$mailuname,$mailpwd,"",$smtp_auth);
	}
}
...
if($activevalue[0] == 1)
{
	...
	while ($myrow = $adb->fetch_array($result))
	{
		...
		$body_content = getTranslatedString('Dear_Team','vtiger','zh_cn').getTranslatedString('Dear_Team_Time_to_Party','vtiger','zh_cn')."<br><br>".getTranslatedString('Potential_Id','vtiger','zh_cn')." ".$pot_id;
		$body_content .= getTranslatedString('Potential_Name','vtiger','zh_cn')." ".$pot_name."<br><br>";
		sendmail($emailaddress,$mailuname,getTranslatedString('Big_Deal_Closed_Successfully','vtiger','zh_cn'),$body_content,$mailserver,$mailuname,$mailpwd,"",$smtp_auth);
	}
}
//Pending tickets
...
if($activevalue[0] == 1)
{
	...
	while ($myrow = $adb->fetch_array($result))
	{
		...
		sendmail($emailaddress,$mailuname,getTranslatedString('Pending_Ticket_notification'),getTranslatedString('Kind_Attention').$ticketid .getTranslatedString('Thank_You_HelpDesk'),$mailserver,$mailuname,$mailpwd,"",$smtp_auth);
	}
}
...
//changes made to get too many tickets notification only when tickets count is greater than or equal to 5
	if($count >= 5)
	{
		sendmail($emailaddress,$mailuname,getTranslatedString('Too_many_pending_tickets'),getTranslatedString('Dear_Admin_too_ many_tickets_pending'),$mailserver,$mailuname,$mailpwd,"",$smtp_auth);
	}
}
...
if($activevalue[0] == 1)
{
	...
	{
		$productname=$myrow[0];
		sendmail($emailaddress,$mailuname,getTranslatedString('Support_starting'),getTranslatedString('Hello_Support').$productname ."\n ".getTranslatedString('Congratulations'),$mailserver,$mailuname,$mailpwd,"",$smtp_auth);
	}
}
...
if($activevalue[0] == 1)
{
	...
	{
		$productname=$myrow[0];
		sendmail($emailaddress,$mailuname,getTranslatedString('Support_Ending_Subject'),getTranslatedString('Support_Ending_Content').$productname.getTranslatedString('kindly_renew'),$mailserver,$mailuname,$mailpwd,"",$smtp_auth);
	}
}
