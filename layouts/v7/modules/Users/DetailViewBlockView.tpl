							...
							<tr>
								{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
									...
									{if $FIELD_MODEL->getName() eq 'theme' or $FIELD_MODEL->getName() eq 'rowheight'}
										{continue}
									{/if}
									{if  $FIELD_MODEL->getName() eq 'department'}
										{assign var="departmentname" value=$DEPARTMENT_MODEL->getInstanceById($FIELD_MODEL->get('fieldvalue'))->get('departmentname')}
										<script>
											$(function() {
												$('#Users_detailView_fieldValue_department').find('span').eq(0).text('{$departmentname}');
												$('#Users_detailView_fieldValue_department').find('span').eq(1).find('input').attr('data-displayvalue','{$departmentname}');
												$('#Users_detailView_fieldValue_department').find('span').eq(1).find('input').attr('data-value','{$departmentname}');
											});
										</script>
									{/if}
