                        ...
                        <div class="mCustomScrollBox mCS-light-2 mCSB_inside" tabindex="0">
                            <div class="mCSB_container" style="position:relative; top:0; left:0;">
                                ...
                                {foreach item="FOLDER" from=$FOLDERS}
                                    ...
                                    <li style="font-size:12px;" class='documentFolder {if $FOLDER_VALUE eq $FOLDER->getName()} active{/if}' id="{$FOLDER->get('folderid')}" okk="parent_{$FOLDER->get('folderid')}">
                                ...
                                {/foreach}
                                    <script>
                                        function parentlist(parentid, tab){
                                            $.ajax({
                                                type: "GET",
                                                ...
                                                success: function(result) {
                                                    $.each( result.result, function(index, content) {
                                                        var html = '';
                                                        var button = '';
                                                        html = '<li style="font-size:12px;" class="documentFolder" id="' + content.folderid + '"  okk="parent_' + content.folderid + '">&nbsp;<a class="filterName" href="javascript:void(0);" data-filter-id="' + content.folderid + '" data-folder-name="' + content.foldername + '" title="' + content.foldername + '">' +
                                                                tab +'|--<i class="fa {if $FOLDER_VALUE eq $FOLDER->getName()}fa-folder-open{else}fa-folder{/if}"></i> <span class="foldername">' + content.foldername + '</span></a></li>';
                                                        button = '<div class="dropdown pull-right"><span class="fa fa-caret-down dropdown-toggle" data-toggle="dropdown" aria-expanded="true"></span>' +
                                                                '<ul class="dropdown-menu dropdown-menu-right vtDropDown" role="menu"><li class="editFolder " data-folder-id="'+ content.folderid +'"><a role="menuitem" ><i class="fa fa-pencil-square-o"></i>&nbsp;Edit</a>' +
                                                                '</li><li class="deleteFolder " data-deletable="{!$FOLDER->hasDocuments()}" data-folder-id="'+ content.folderid +'"><a role="menuitem" ><i class="fa fa-trash"></i>&nbsp;Delete</a></li></ul></div>';
                                                        $('#folders-list').find("li[okk=parent_" + parentid + "]").after(html);
                                                        $('#folders-list').find("li[okk=parent_" + parentid + "]").after(button);
                                                        parentlist(content.folderid,tab+"&nbsp;&nbsp;&nbsp;&nbsp;");
                                                    });
                                                }
                                            });
                                        }
