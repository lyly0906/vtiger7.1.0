...
Vtiger_List_Js("Books_List_Js", {
	recordSelectTrackerInstance: false,
	emptyBooks: function (url) {
		var message = app.vtranslate('JS_MSG_EMPTY_RB_CONFIRMATION');
		app.helper.showConfirmationBox({'message': message}).then(
			function (e) {
				var deleteURL = url + '&mode=emptyBooks';
				var instance = new Books_List_Js();
				app.request.post({url: deleteURL}).then(
						function (error, data) {
							if (!error) {
								instance.BooksActionPostOperations(data);
							} else {
	...
	deleteRecord: function (recordId) {
		var recordId = Books_List_Js.convertToJsonString(recordId);
		var listInstance = Vtiger_List_Js.getInstance();
	...
	restoreRecord: function (recordId) {
		...
		app.helper.showConfirmationBox({'message': message}).then(
			function (e) {
				Books_List_Js.restoreAction(recordId, false);
			},
			function (error, err) {
			});
	}
	...
	getRecordSelectTrackerInstance: function () {
		if (Books_List_Js.recordSelectTrackerInstance === false) {
			Books_List_Js.recordSelectTrackerInstance = Vtiger_RecordSelectTracker_Js.getInstance();
		}
		return Books_List_Js.recordSelectTrackerInstance;
	},
	...
	listViewPostOperation: function () {
		if (!jQuery('#isRecordsDeleted').val()) {
			jQuery(".clearBooks").attr('disabled', 'disabled');
		} else {
			jQuery(".clearBooks").removeAttr('disabled');
		}
	},
	...
	/*
	 * Function to perform the operations after the Empty Books
	 */
	BooksActionPostOperations: function (data) {
	...
	registerDeleteRecordClickEvent: function () {
		...
		listViewContentDiv.on('click', '.deleteRecordButton', function (e) {
			...
			Books_List_Js.deleteRecord(recordId);
			e.stopPropagation();
		});
	},
	/*
	...
	registerRestoreRecordClickEvent: function () {
		...
		listViewContentDiv.on('click', '.restoreRecordButton', function (e) {
			var elem = jQuery(e.currentTarget);
			var recordId = elem.closest('tr').data('id');
			Books_List_Js.restoreRecord(recordId);
			e.stopPropagation();
		});
	},
