            ...
            <form name='list' id='listedit' action='' onsubmit="return false;">
                <table id="listview-table"  class="table {if $LISTVIEW_ENTRIES_COUNT eq '0'}listview-table-norecords {else} listview-table{/if} ">
                    ...
                    {foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
                        {if $LISTVIEW_HEADER->getName() eq 'email1' or $LISTVIEW_HEADER->getName() eq 'first_name' or $LISTVIEW_HEADER->getName() eq 'roleid' or $LISTVIEW_HEADER->getName() eq 'phone_work' or $LISTVIEW_HEADER->getName() eq 'department'}
                        <th>
                                    ...
                                    {if $LISTVIEW_HEADER->getName() eq 'email1' or $LISTVIEW_HEADER->getName() eq 'first_name' or $LISTVIEW_HEADER->getName() eq 'roleid' or $LISTVIEW_HEADER->getName() eq 'phone_work' or $LISTVIEW_HEADER->getName() eq 'department'}
                                        {if $LISTVIEW_HEADER->getName() eq 'first_name'}
                                            <td class="listViewEntryValue" data-name="{$LISTVIEW_HEADER->get('name')}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}">
                                                <span class="fieldValue">
                                                    <span class="value">
                                                        {showname last_name=$LISTVIEW_ENTRY->get('last_name') first_name=$LISTVIEW_ENTRY_VALUE}
                                                    </span>
                                                </span>
                                        ...
                                        {else}
                                            <td class="listViewEntryValue" data-name="{$LISTVIEW_HEADER->get('name')}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}">
                                                <span class="fieldValue">
                                                    <span class="value">
                                                        {if $LISTVIEW_HEADER->getName() neq 'email1'}
                                                            {if $LISTVIEW_HEADER->getName() eq 'department'}
                                                                {$DEPARTMENT_MODEL->getInstanceById($LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME))->get('departmentname')}
                                                            {else}
                                                                {$LISTVIEW_ENTRY_VALUE|strip_tags}
                                                            {/if}
                                                        {else}
