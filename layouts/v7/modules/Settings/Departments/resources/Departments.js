...
var Settings_Departments_Js = {
			...
			submitHandler : function(form) {
				...
				app.request.post({'data' : formData}).then(function(err, data){
					...
					if(err === null){
						...
						Settings_Departments_Js.initTreeView();
					}
				});
	...
	registerShowNewProfilePrivilegesEvent : function() {
		jQuery('[name="profile_directly_related_to_role"]').on('change',function(e){
			...
			if(hanlder === 'new'){
				Settings_Departments_Js.getProfilePriviliges();
				return false;
			}else {
				jQuery('#profilesList', container).removeClass('hide');
                Settings_Departments_Js.registerProfileEvents();
			}
				...
				if(form.data('submit') == 'true' && form.data('performCheck') == 'true') {
					...
					if(this.numberOfInvalids() <= 0) {
						...
						thisInstance.checkDuplicateName({
							...
							'parentid':formData.parent_roleid,
							'parentname':formData.parent_roleid_display
						}).then(
							function(data){
								app.helper.showProgress();
...
Vtiger.Class("Settings_Departments_Index_Js",{},{
	init : function() {
		this.addComponents();
...
Vtiger.Class("Settings_Departments_Edit_Js",{},{
	init : function() {
		this.addComponents();
                Settings_Departments_Js.registerEvents();
	},
