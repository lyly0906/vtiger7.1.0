				...
				<form class="form-horizontal recordEditView" id="EditView" name="edit" method="post" action="index.php" enctype="multipart/form-data">
					...
					<div class="editViewBody">
						<div class="editViewContents">
							...
							{/if}
                            {* ITS4You start of content change  JK *}
                            {if $smarty.request.view eq 'EditNumbering'}
							...
							{/if}
                            {if $smarty.request.view eq 'EditNumbering'}
                                {include file="partials/EditNumberingViewContents.tpl"|@vtemplate_path:'ITS4YouMultiCompany'}
							{else}
                                {include file="partials/EditViewContents.tpl"|@vtemplate_path:$MODULE}
                            {/if}
						</div>
					</div>
                    {if $smarty.request.view neq 'EditNumbering'}
