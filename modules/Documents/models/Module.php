...
class Documents_Module_Model extends Vtiger_Module_Model {
	...
	public static function getAllFolders() {
		...
		$result = $db->pquery('SELECT * FROM vtiger_attachmentsfolder where parentid is null ORDER BY sequence', array());
	...
	public static function getFoldersByParentid($id) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM vtiger_attachmentsfolder where parentid='.$id, array());
		$folderList = array();
		for($i=0; $i<$db->num_rows($result); $i++) {
			$row = $db->query_result_rowdata($result, $i);
			$folderList[] = Documents_Folder_Model::getInstanceByArray($row);
