...
class Books_List_View extends Vtiger_Index_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		$moduleModel = Books_Module_Model::getInstance($moduleName);
	...
	public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer) {
        ...
        file_put_contents("log.txt", "-----searchParams-----1--".var_export($searchParams,true), FILE_APPEND);
		if($request->get('groupid')){
			$groupId = Settings_Departments_Record_Model::getInstanceByName($request->get('groupid'),'',$request->get('companyid'))->getId();
		}

		if($groupId && $searchParams){
		    $department =  array (
                0 => 'department',
                1 => 'c',
                2 => $groupId,
            );
            $search = $searchParams[0][0];
            $search.array_push($department);
        }else if($groupId && !$searchParams){
            $searchParams = array(
                array(
                    array('department','c',$groupId)
                )
            );
        }else if(!$groupId){
			$searchParams = array(
        ...
        file_put_contents("log.txt", "-----searchParams-----2---".var_export($searchParams,true), FILE_APPEND);
		...
		$listViewModel = Books_ListView_Model::getInstance($moduleName, $sourceModule);
		$linkParams = array('MODULE'=>$moduleName, 'ACTION'=>$request->get('view'));
		...
		$viewer->assign('DEPARTMENT_MODEL', new Settings_Departments_Record_Model());
		$viewer->assign('LISTVIEW_ENTRIES_COUNT',$noOfEntries);
		$viewer->assign('LISTVIEW_HEADERS', $this->listViewHeaders);

		$viewer->assign('LISTVIEW_ENTRIES', $this->listViewEntries);
		$viewer->assign('MODULE_LIST', $moduleModel->getAllGroupList());
        $rootDepartment = Settings_Departments_Record_Model::getBaseDepartment();
        $viewer->assign('ROOT_DEPARTMENT', $rootDepartment);
	...
	public function setModuleInfo($request, $moduleModel) {
		...
		$moduleModel = Books_Module_Model::getInstance($request->getModule());
		//If sourceModule is empty, pick the first module name from the list
		if (empty($sourceModule)) {
	...
	public function getHeaderCss(Vtiger_Request $request) {
	 ...
	 function getzhcn($arr){
	    $str = $arr['first_name'].$arr['last_name'];
