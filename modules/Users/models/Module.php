...
class Users_Module_Model extends Vtiger_Module_Model {
	...
	public function checkDuplicateUserPhone($phone){
		$status = false;
		// To check username existence in db
		$db = PearDatabase::getInstance();
		$query = 'SELECT phone_mobile FROM vtiger_users WHERE phone_mobile = ?';
		$result = $db->pquery($query, array($phone));
		if ($db->num_rows($result) > 0) {
			$status = true;
		}
		return $status;
	...
	public function checkDuplicateUserEmail($email){
		...
		// To check username existence in db
		$db = PearDatabase::getInstance();
		$query = 'SELECT email1 FROM vtiger_users WHERE email1 = ?';
