...
class Users_Record_Model extends Vtiger_Record_Model {
	...
	public function getUserInfo() {
		$db = PearDatabase::getInstance();
		$query = 'SELECT first_name,last_name,roleid,department,phone_mobile FROM vtiger_users LEFT JOIN vtiger_user2role ON vtiger_users.id = vtiger_user2role.userid WHERE id = ?';
		$result = $db->pquery($query, array($this->getId()));
		$name = [];
		if($db->num_rows($result) > 0){
			$name['last_name'] = $db->query_result($result, 0,  'last_name');
			$name['first_name'] = $db->query_result($result, 0, 'first_name');
			$name['roleid'] = $db->query_result($result, 0, 'roleid');
			$name['department'] = $db->query_result($result, 0, 'department');
			$name['phone_mobile'] = $db->query_result($result, 0, 'phone_mobile');
		}
		return $name;
	}
	...
	public static function isEmailExists($email) {
		...
		$status = false;
		// To check username existence in db
		return $userModuleModel->checkDuplicateUserEmail($email);
	}
	...
	public static function isPhoneExists($phone) {
		...
		$status = false;
		// To check username existence in db
		return $userModuleModel->checkDuplicateUserPhone($phone);
	}

	public function getDepartmentIdByDepartmentName($name,$its4you_company = 0){
		$db = PearDatabase::getInstance();
		$query = "select departmentid from vtiger_department gr
                    where departmentname = ? and its4you_company = ?  limit 1";
		$result = $db->pquery($query, array($name,$its4you_company));
		$resultDepartmentcount = $db->num_rows($result);
		if($resultDepartmentcount>0){
			for($j=0; $j<$resultDepartmentcount; ++$j) {
				$department_id = $db->query_result($result, 0, 'departmentid');
			}
		}
