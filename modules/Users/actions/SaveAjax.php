...
class Users_SaveAjax_Action extends Vtiger_SaveAjax_Action {
	...
	function __construct() {
		...
		$this->exposeMethod('phoneExists');
		$this->exposeMethod('emailExists');
	...
	public function phoneExists(Vtiger_Request $request){
		$module = $request->getModule();
		$phone = $request->get('phone_mobile');
		$status = Users_Record_Model::isPhoneExists($phone);
		$response = new Vtiger_Response();
		$response->setResult($status);
		$response->emit();
	}

	public function emailExists(Vtiger_Request $request){
		$module = $request->getModule();
		$email = $request->get('email1');
		$status = Users_Record_Model::isEmailExists($email);
		$response = new Vtiger_Response();
		$response->setResult($status);
		$response->emit();
	}
	...
	public function savePassword(Vtiger_Request $request) {
		...
		if ($wsStatus['message']) {
			//修改chat的密码
			$userM = new Users_Record_Model();
			$userM->set('id', $request->get('userid'));
			$userinfo = $userM->getUserInfo();

			$originUrl = $chat_URL.'user/queryByPhone';
			$post = array('phone' => $userinfo['phone_mobile']);
			...
			file_put_contents("log.txt", "-----user-get--".var_export($userId, true), FILE_APPEND);
			if($userId){
				$originUrl = $chat_URL.'user/password/update';
				$isadmin = 0;
				if(is_admin($userModel)){
					$isadmin = 1;
				}
				$post = array('userId' => $userId, 'oldPassword' => $oldPassword, 'newPassword' => $newPassword, 'isadmin' => $isadmin);
				$client = new Vtiger_Net_Client($originUrl);
				$clientresut = $client->doPost($post);
				file_put_contents("log.txt", "-----user-modify--".var_export($clientresut, true), FILE_APPEND);
			}
